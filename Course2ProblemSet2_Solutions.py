...
class UndirectedGraph:
    ...
    def dfs_visit(self, i, dfs_timer, discovery_times, finish_times, 
        ...
        dfs_timer.increment()
        # your code here
        for elt in self.adj_list[i]:
            if(discovery_times[elt] == None):
                dfs_tree_parent[elt] = i
                self.dfs_visit(elt, dfs_timer, discovery_times, finish_times, dfs_tree_parent, dfs_back_edges)
                dfs_timer.increment()
            else:
                if(discovery_times[elt] != None and finish_times[elt] == None):
                    dfs_back_edges.append((i, elt))
...
def find_all_nodes_in_cycle(g): # g is an UndirectedGraph class
    ...
    for (i, j) in non_trivial_back_edges:
        set_of_nodes.add(i)
        set_of_nodes.add(j)
        for x in g.adj_list[i]:
            if x in g.adj_list[j]:
                set_of_nodes.add(x)
    return set_of_nodes
